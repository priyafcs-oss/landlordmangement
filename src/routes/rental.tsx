 ...
 * statements or lease agreements the landlord has as a file rather than an email attachment.
 */
function UploadDocumentDialog() {
  ...
  const upload = async () => {
    ...
    try {
      ...
      if (data.skipped) {
        toast.info("Uploaded, but it didn't look like a bill, lease or rent statement — nothing recorded.");
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Upload a bill, rent statement or lease</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <p className="text-xs text-muted-foreground">
            Same AI reader as the email inbox — a PDF or photo of a council/water bill, agent rent statement, or
            lease agreement. It's matched to a property and staged the same way an emailed document would be.
          </p>
