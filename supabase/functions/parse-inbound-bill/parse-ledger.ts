...
- expenseLines is REQUIRED — always include it, even as an empty array [] when there's nothing to report. A managing agent's ownership/disbursement statement typically deducts its own management fee, and sometimes pays a bill on the owner's behalf before remitting the balance — extract each such deduction as one expenseLine: vendor (who it was paid to — the agent itself for its own fee, or the actual biller for a bill they paid), amount (positive number), date (YYYY-MM-DD), description, category (a short free-text tax category like "management_fees" or "water_rates"), and tenantName.
- tenantName on an expenseLine is REQUIRED as a key but its value is nullable — this statement may cover MULTIPLE dwellings/tenancies at one property (e.g. a house and a granny flat, each with its own tenant and its own rent/deductions section or column). Whenever a deduction is printed under, or clearly tied to, one specific tenancy's own section/heading (most commonly a management fee, which is normally charged as a percentage of THAT tenancy's own rent) — set tenantName to that section's tenant name, exactly as printed there, even if it differs from the top-level tenantName or from names elsewhere on the statement. Leave it null only when a deduction is genuinely whole-of-property (e.g. a single water/council bill covering the entire property, not split by dwelling) or the statement gives no way to attribute it to one tenancy.
  ...
  properties: {
    ...
    expenseLines: {
      ...
      items: {
        ...
        required: ["vendor", "amount", "date", "description", "category", "tenantName"],
      },
    },
...
export async function parseRentStatement(
  ...
  const row = {
    ...
    payload: {
      ...
      expenseLines: (parsed.expenseLines ?? []).map((e) => ({ ...e, tenantName: e.tenantName ?? undefined })),
