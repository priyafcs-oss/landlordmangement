import { extractLoanDocumentFields } from "../parse-inbound-bill/parse-loan-document.ts";
...
/**
 * Stateless extraction-only endpoint for the "Add Loan" dialog's upload — reads a new loan's
 * offer/contract/approval letter via Gemini (same prompt/schema as the email-ingestion pipeline)
 * and returns the parsed fields for the form to pre-fill. Unlike parse-inbound-bill, this never
 * writes to the database: the landlord reviews/edits the pre-filled form and hits Save themselves.
 */
Deno.serve(async (req) => {
  ...
  if (!Deno.env.get("GEMINI_API_KEY")) {
    console.error("[extract-loan-document] GEMINI_API_KEY is not configured");
  ...
  try {
    const parsed = await extractLoanDocumentFields({
      subject: `Loan document upload: ${body.fileName}`,
    ...
    console.error("[extract-loan-document] unhandled error", e);
